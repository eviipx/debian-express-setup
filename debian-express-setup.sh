...
setup_containers() {
  ...
  # Offer to install Docker Compose
  if whiptail --title "Docker Compose" --yesno "Do you want to install Docker Compose?" 8 60; then
  ...
  if whiptail --title "Portainer" --yesno "Do you want to install Portainer for container management?" 8 60; then
    ...
    record_installed_service "portainer" "9443"
    msg_ok "Portainer installed and running on port 9443"
  fi
}
...
main() {
  ...
  msg_ok "Setup complete! Your system has been configured and optimized."
  msg_info "Please reboot your system to apply all changes."
}
